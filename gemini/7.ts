...
const loginUser = (req: Request, res: Response, next: NextFunction, user: UserDocument): void => {
  req.logIn(user, (err) => {
    if (err) {
      return next(err);
    }
...
export const postUpdateProfile = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  User.findById(user.id, (err: NativeError, foundUser: UserDocument) => {
    if (err) {
      return next(err);
    }
    ...
    foundUser.save((err: WriteError & CallbackError) => {
      if (err) {
        if (err.code === 11000) {
          req.flash("errors", {
...
export const postForgot = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  async.waterfall(
    [
      ...
      (token: AuthToken, done: (err: NativeError | WriteError, token?: AuthToken, user?: UserDocument) => void) => {
      ...
      (token: AuthToken, user: UserDocument, done: (err: Error) => void) => {
        const transporter = nodemailer.createTransport({
          service: "SendGrid",
          auth: {
            user: process.env.SENDGRID_USER,
